document.addEventListener('DOMContentLoaded', function () {
    ...
    /* =======================
       Funciones para actualizar fecha actual y verificar citas
    ======================= */
    ...
    function checkTodayAppointments() {
        ...
        checkAppointmentsForDate(today);
    }

    /* =======================
       Función para verificar citas en una fecha específica
    ======================= */
    function checkAppointmentsForDate(date) {
        // Obtener los eventos para esa fecha específica
        const dateEvents = calendar.getEvents().filter(event => {
            // Comparar si el evento está en la fecha seleccionada
        ...
        if (appointmentBtn) {
            // Mostrar el botón solo si hay eventos ese día
    ...
    /* =======================
       Función para cargar la emoción para una fecha
    ======================= */
    function loadEmotionForDate(date) {
        // Esta función debe implementarse según tus necesidades
        // Por ejemplo, podrías cargar emociones desde localStorage
        console.log('Cargando emoción para la fecha:', date);
    }
    ...
    try {
        ...
        // Añadir el nuevo evento de clic en fecha
        calendar.on('dateClick', function(info) {
            const selectedDate = info.date;
            // Actualizar el título con la fecha seleccionada
            const options = { day: 'numeric', month: 'long' };
            const formattedDate = selectedDate.toLocaleDateString('es-ES', options).toUpperCase();
            const dateElement = document.getElementById('current-date');
            if (dateElement) {
                dateElement.textContent = formattedDate;
            }
            ...
            // Cargar la emoción para la fecha seleccionada
            loadEmotionForDate(selectedDate);
        });
        ...
        // Inicializar datos para la fecha actual
        updateCurrentDate();
        checkTodayAppointments();
        loadEmotionForDate(new Date());
